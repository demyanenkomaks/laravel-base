...
{
    protected static ?string $pluralModelLabel = 'Отправленные E-mail';
    ...
    {
        return config('filament-email.resource.model') ?? \RickDBCN\FilamentEmail\Models\Email::class;
    }
            ...
            Filter::make('headers-filter')
                ...
                ->query(function (Builder $query, array $data): Builder {
                    return $query
                        ->when(
                            $data['from'],
                            fn (Builder $query, $value): Builder => $query->where('from', 'like', sprintf('%%%s%%', $value)),
                        )
                        ->when(
                            $data['to'],
                            fn (Builder $query, $value): Builder => $query->where('to', 'like', sprintf('%%%s%%', $value)),
                        )
                        ->when(
                            $data['cc'],
                            fn (Builder $query, $value): Builder => $query->where('cc', 'like', sprintf('%%%s%%', $value)),
                        )
                        ->when(
                            $data['bcc'],
                            fn (Builder $query, $value): Builder => $query->where('bcc', 'like', sprintf('%%%s%%', $value)),
                        )
                        ->when(
                            $data['attachments'],
                            // JSON_LENGTH
